...
class WxPayConf_pub(object):
    ...
    if settings.TEST_ENV:
        # 测试号.
        APPID = "wx38938db310945275"
        APPSECRET = "5f2d4f5e6bca442b24995525af4f9a80"
        MCHID = ""
        KEY = ""
        ...
        APPID = "wx00795021b3e24138"
        APPSECRET = "1c5cca1d20b325b3ac3e13238a24fd0e"
#         APPID = "wxbefd048ed2f300c6"
#         APPSECRET = "a4740a0e77cd0758b844e84c97de7a77"
        MCHID = "1231497702"
        KEY = "KM5UG2Qdmz0SsVwjnqKxAtjCoiIiIMSO"
        SSLCERT_PATH = "{}/weixin/cacert/prod/apiclient_cert.pem".format(BASE_PATH)
        SSLKEY_PATH = "{}/weixin/cacert/prod/apiclient_key.pem".format(BASE_PATH)
...
class CurlClient(object):
    ...
    def post(self, url, second=30):
        return self.postXmlSSL(None, url, second=second, cert=False, post=True)
...
class RequestsClient(object):
    ...
    def postXml(self, url, data, timeout=15):
        """不使用证书"""
        return self.postXmlSSL(url, data, cert=False, post=True)
...
class JsApi_pub(Common_util_pub):
    ...
    def createOauthUrlForCode(self, redirectUrl):
        ...
        urlObj["redirect_uri"] = quote(redirectUrl, '')     # quote(redirectUrl, '')
    ...
    def getOpenid(self):
        ...
        self.openid = json.loads(data.decode('gb2312')).get('openid', None)
        return self.openid
...
class Redpack_pub(Wxpay_client_pub):
    ...
    def createXml(self):
        ...
        if self.parameters["re_openid"] is None:
            raise ValueError("redpack need openid parameters")

        self.parameters['mch_id'] = WxPayConf_pub.MCHID
        self.parameters['wxappid'] = WxPayConf_pub.APPID
        self.parameters['client_ip'] = '222.73.202.178'
        self.parameters["nonce_str"] = self.createNoncestr()
        self.parameters["total_num"] = '1'
        self.parameters["logo_imgurl"] = 'https://wx.gtimg.com/mch/img/ico-logo.png'
    ...
    def postXmlSSL(self):
        """使用证书post请求xml"""
        xml = self.createXml()
        cert = WxPayConf_pub.SSLCERT_PATH
        key = WxPayConf_pub.SSLKEY_PATH
        headers = {'Content-Type': 'application/xml'}
        self.response = requests.post(self.url, data=xml.encode(), verify=True, cert=(cert, key), headers=headers).text
        return self.response
